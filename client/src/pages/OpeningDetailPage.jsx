...
function OpeningDetailPage() {
  ...
  return <div className="page detail-page"><Link className="back-link" to="/openings">← Open positions</Link><PageHeader eyebrow={opening.department} title={opening.title} description={opening.description} /><div className="detail-tabs">{['Overview', 'Pipeline', 'Activity'].map((item) => <button className={tab === item ? 'detail-tab active' : 'detail-tab'} type="button" key={item} onClick={() => setTab(item)}>{item}</button>)}</div>{tab === 'Overview' && <section className="detail-overview"><div><p className="section-kicker">Position</p><h2>{opening.title}</h2><p>{opening.description}</p></div><StatusIndicator label={opening.status} tone={opening.status === 'open' ? 'success' : 'warning'} /></section>}{tab === 'Activity' && <EmptyState title="Opening activity" description="Candidate activity is available from each application timeline." />}{tab === 'Pipeline' && <div className="kanban-board">{stages.map((stage) => <section className="kanban-column" key={stage}><div className="kanban-heading"><h2>{stage}</h2><span>{grouped[stage].length}</span></div>{grouped[stage].map((application) => <Link className="kanban-card" to={`/candidates/${application._id}`} key={application._id}><strong>{application.candidateName}</strong><span>{application.source}</span><small>{daysInStage(application.stageEnteredAt)}d in stage</small></Link>)}{!grouped[stage].length && <p className="column-empty">No candidates</p>}</section>)}</div>}</div>
}
